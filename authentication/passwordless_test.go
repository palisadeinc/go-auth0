...
func TestSendEmail(t *testing.T) {
	...
	r, err := authAPI.Passwordless.SendEmail(
		context.Background(), passwordless.SendEmailRequest{
...
func TestLoginWithEmail(t *testing.T) {
	...
	token, err := authAPI.Passwordless.LoginWithEmail(
		context.Background(), passwordless.LoginWithEmailRequest{
			Code:     "123456",
			Email:    "test-email@example.com",
...
func TestSendSMS(t *testing.T) {
	...
	r, err := authAPI.Passwordless.SendSMS(
		context.Background(), passwordless.SendSMSRequest{
...
func TestLoginWithSMS(t *testing.T) {
	...
	token, err := authAPI.Passwordless.LoginWithSMS(
		context.Background(), passwordless.LoginWithSMSRequest{
			PhoneNumber: "+123456789",
			Code:        "123456",
			Scope:       "openid profile email offline_access",
			Audience:    "https://api.example.com",
...
func TestPasswordlessWithIDTokenVerification(t *testing.T) {
	t.Run(
		"error for an invalid organization when using org_id", func(t *testing.T) {
			...
			_, err = api.Passwordless.LoginWithEmail(
				context.Background(), passwordless.LoginWithEmailRequest{
					Code:     "123456",
					Email:    "test-email@example.com",
					Scope:    "openid profile email offline_access",
	...
	t.Run(
		"error for an invalid maxage", func(t *testing.T) {
			...
			_, err = api.Passwordless.LoginWithSMS(
				context.Background(), passwordless.LoginWithSMSRequest{
					PhoneNumber: "+123456789",
					Code:        "123456",
					Scope:       "openid profile email offline_access",
...
func TestPasswordlessWithClientAssertion(t *testing.T) {
	t.Run(
		"Should support using private key jwt auth", func(t *testing.T) {
			...
			r, err := api.Passwordless.SendSMS(
				context.Background(), passwordless.SendSMSRequest{
					PhoneNumber: "+123456789",
				},
			)
